# Add Claude Desktop icon and .desktop file based on Debian package implementation
{ config, pkgs, lib, ... }:

let
  claudeIconPackage = pkgs.stdenvNoCC.mkDerivation {
...
in
{
  # Add the icon package to the system packages
  environment.systemPackages = [ claudeIconPackage ];

  # Update icon caches after installations
  system.activationScripts.claude-desktop-icon = ''
    if [ -x "${pkgs.gtk3}/bin/gtk-update-icon-cache" ]; then
      ${pkgs.gtk3}/bin/gtk-update-icon-cache -f -t ${pkgs.hicolor-icon-theme}/share/icons/hicolor
    fi

    if [ -x "${pkgs.desktop-file-utils}/bin/update-desktop-database" ]; then
      ${pkgs.desktop-file-utils}/bin/update-desktop-database -q /run/current-system/sw/share/applications
    fi
  '';
}
